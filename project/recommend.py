...
def group_by_centroid(restaurants, centroids):
    ...
    for restaurant in restaurants:
        ...
        closest = find_closest(location,centroids)
        allPairs.append([closest, restaurant])
    return group_by_first(allPairs)
...
def find_centroid(cluster):
    ...
    for restaurant in cluster:
        location = restaurant_location(restaurant)
        longtitude.append(location[0])
        latitude.append(location[1])
    return [mean(longtitude), mean(latitude)]
...
def find_predictor(user, restaurants, feature_fn):
    ...
    syy = sum((j - mean_y) ** 2 for j in ys)
    zipped = zip(xs, ys)
    sxy = sum ((i[0] - mean_x) * (i[1] - mean_y) for i in zipped)
    b = sxy / sxx
    a = mean_y - b * mean_x
...
def best_predictor(user, restaurants, feature_fns):
    ...
    best_fn, best_Rsquared = max(predictor_list, key = lambda x: x[1])
    return best_fn
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    for restaurant in restaurants:
        ...
        if restaurant in reviewed:
            ...
            dic.update({name: rating})
            
        else:
            dic.update({name : predictor(restaurant)})
